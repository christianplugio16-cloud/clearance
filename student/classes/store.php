...
public static function registerStudent() {
    ...
    // Insert student record with pending status (requires admin approval)
    $stmt = $conn->db->prepare("INSERT INTO account_studentprofile 
                                ...
                                VALUES (:fullname, :username, :password, :email, :dept, :session, 'pending', :created_on)");
    $stmt->bindParam(':fullname', $fullname, PDO::PARAM_STR);
    $stmt->bindParam(':username', $username, PDO::PARAM_STR);
    ...
    if ($stmt->execute()) {
        return array('status' => 1, 'message' => 'Registration successful! Please wait for admin approval before you can login.');
    } else {
